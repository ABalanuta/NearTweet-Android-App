...
public class ConnectionHandlerService extends Service {
	...
	public void sendTweet(TweetDTO tweet){
		...
		}else{ Log.e("ServiceP", "Channel is Closed"); }
	}
	...
	/* Keeps listening for new tweets */
				...
				if(isConnected() && hasTweets()){
					...
					for(BasicDTO dto : list){
						...
						if(dto.getType().equals(TypeofDTO.TWEET_DTO)){
							...
							Log.i("GPS FIX", "RECEIVED NEW TWEET AND IT HAS THIS COORDINATES LAT: " + t.getLAT() + " LNG: " + t.getLNG());
						...
						else{ Log.e("ServiceP", "There is no Such Type of Tweet : "  + dto.getType()); }
					}
				}
				else{
